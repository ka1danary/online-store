const uuid = require('uuid') // для генерации уникального id
const path = require('path')
const {Device, DeviceInfo} = require('../models/models') // для создания девайса
...
class deviceController {
    ...
    async create(req,res, next) { // создание девайса
        try {
            ...
            // получение изображения - установка npm install express-fileupload
            const {img} = req.files
            ...
            //после перемещения файла создадим сам девайс
            const device = await Device.create({name, price, brandId, typeId, img: fileName}) // в фото передаем название файла
